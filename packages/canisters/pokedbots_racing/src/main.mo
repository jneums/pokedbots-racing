...
import GarageCombineParts "tools/garage_combine_parts";
import GarageListBatteries "tools/garage_list_batteries";
import GarageJoltBot "tools/garage_jolt_bot";
import GarageRepairBattery "tools/garage_repair_battery";
import GarageRebuildBattery "tools/garage_rebuild_battery";
import GarageSalvageBattery "tools/garage_salvage_battery";
import GarageGetStationStatus "tools/garage_get_station_status";
...
//       // Migrate entries
//       let newEntries : [RacingSimulator.RaceEntry] = Array.map<{
//         nftId : Text;
//         owner : Principal;
//         entryFee : Nat;
//         enteredAt : Int;
...
//         oldRace.entries,
//         func(e) : RacingSimulator.RaceEntry {
//           {
//             nftId = e.nftId;
//             owner = e.owner;
//             entryFee = e.entryFee;
...
//             results,
//             func(r) : RacingSimulator.RaceResult {
//               {
//                 nftId = r.nftId;
//                 owner = r.owner;
//                 position = r.position;
...
// Migration: Add #ConsumableProc variant to RaceEventType in stored events
// (Already deployed — commenting out post-migration)
// (
//   with migration = func(
//     old : {
//       stable_races : Map.Map<Nat, {
//         raceId : Nat; name : Text; distance : Nat; terrain : RacingSimulator.Terrain;
//         trackId : Nat; trackSeed : Nat; raceClass : RacingSimulator.RaceClass;
//         entryFee : Nat; maxEntries : Nat; minEntries : Nat; startTime : Int;
//         duration : Nat; entryDeadline : Int; createdAt : Int;
//         entries : [RacingSimulator.RaceEntry]; status : RacingSimulator.RaceStatus;
//         results : ?[RacingSimulator.RaceResult];
//         events : [{
//           eventType : {
//             #Overtake : { overtaker : Text; overtaken : Text };
//             #LeadChange : { newLeader : Text; previousLeader : Text };
//             #LargeGap : { leader : Text; gapSeconds : Float };
//             #CloseRacing : { bots : [Text]; gapSeconds : Float };
//             #ExceptionalPerformance : { bot : Text; performancePct : Float };
//             #PoorPerformance : { bot : Text; performancePct : Float };
//             #SegmentComplete : { segmentIndex : Nat; leader : Text };
//             #LuckProc : { bot : Text; procType : Text; boost : Float };
//             #BadLuck : { bot : Text; incidentType : Text; penalty : Float };
//           };
//           timestamp : Float;
//           segmentIndex : Nat;
//           description : Text;
//         }];
//         prizePool : Nat; platformTax : Nat; platformBonus : Nat;
//         sponsors : [RacingSimulator.Sponsor];
//       }>;
//     }
//   ) : {
//     stable_races : Map.Map<Nat, RacingSimulator.Race>;
//   } {
//     Debug.print("Migration: Adding #ConsumableProc to RaceEventType");
//
//     let migratedRaces = Map.new<Nat, RacingSimulator.Race>();
//     var raceCount = 0;
//
//     for ((raceId, oldRace) in Map.entries(old.stable_races)) {
//       let newEvents = Array.map(oldRace.events, func(e : { eventType : { #Overtake : { overtaker : Text; overtaken : Text }; #LeadChange : { newLeader : Text; previousLeader : Text }; #LargeGap : { leader : Text; gapSeconds : Float }; #CloseRacing : { bots : [Text]; gapSeconds : Float }; #ExceptionalPerformance : { bot : Text; performancePct : Float }; #PoorPerformance : { bot : Text; performancePct : Float }; #SegmentComplete : { segmentIndex : Nat; leader : Text }; #LuckProc : { bot : Text; procType : Text; boost : Float }; #BadLuck : { bot : Text; incidentType : Text; penalty : Float } }; timestamp : Float; segmentIndex : Nat; description : Text }) : RacingSimulator.RaceEvent {
//         let newEventType : RacingSimulator.RaceEventType = switch (e.eventType) {
//           case (#Overtake(v)) { #Overtake(v) };
//           case (#LeadChange(v)) { #LeadChange(v) };
//           case (#LargeGap(v)) { #LargeGap(v) };
//           case (#CloseRacing(v)) { #CloseRacing(v) };
//           case (#ExceptionalPerformance(v)) { #ExceptionalPerformance(v) };
//           case (#PoorPerformance(v)) { #PoorPerformance(v) };
//           case (#SegmentComplete(v)) { #SegmentComplete(v) };
//           case (#LuckProc(v)) { #LuckProc(v) };
//           case (#BadLuck(v)) { #BadLuck(v) };
//         };
//         { eventType = newEventType; timestamp = e.timestamp; segmentIndex = e.segmentIndex; description = e.description };
//       });
//
//       ignore Map.put(migratedRaces, Map.nhash, raceId, {
//         raceId = oldRace.raceId; name = oldRace.name; distance = oldRace.distance;
//         terrain = oldRace.terrain; trackId = oldRace.trackId; trackSeed = oldRace.trackSeed;
//         raceClass = oldRace.raceClass; entryFee = oldRace.entryFee;
//         maxEntries = oldRace.maxEntries; minEntries = oldRace.minEntries;
//         startTime = oldRace.startTime; duration = oldRace.duration;
//         entryDeadline = oldRace.entryDeadline; createdAt = oldRace.createdAt;
//         entries = oldRace.entries; status = oldRace.status;
//         results = oldRace.results; events = newEvents;
//         prizePool = oldRace.prizePool; platformTax = oldRace.platformTax;
//         platformBonus = oldRace.platformBonus; sponsors = oldRace.sponsors;
//       });
//       raceCount += 1;
//     };
//
//     Debug.print("Migrated events for " # Nat.toText(raceCount) # " races");
//     { stable_races = migratedRaces };
//   }
// )

// Migration: Add boundToBot field to GearPiece for soulbound gear system
// (Already deployed — commenting out post-migration)
// (
//   with migration = func(
//     old : {
//       stable_gear_pieces : Map.Map<Nat, {
//         gearId : Nat;
//         name : Text;
//         description : Text;
//         slot : GearSystem.GearSlot;
//         rarity : GearSystem.GearRarity;
//         category : GearSystem.GearCategory;
//         terrainTag : GearSystem.TerrainTag;
//         ilvl : Nat;
//         season : Nat;
//         speedBonus : Nat;
//         powerCoreBonus : Nat;
...
//         stabilityBonus : Nat;
//         luckBonus : Nat;
//         passive : ?GearSystem.PassiveEffect;
//         // NOTE: boundToBot field is missing in old data
//         sourceRaceId : ?Nat;
...
//     }
//   ) : {
//     stable_gear_pieces : Map.Map<Nat, GearSystem.GearPiece>;
//   } {
//     Debug.print("Migration: Adding boundToBot field to GearPiece (soulbound gear)");
//
//     let migratedGear = Map.new<Nat, GearSystem.GearPiece>();
//     var count = 0;
//
//     for ((gearId, oldGear) in Map.entries(old.stable_gear_pieces)) {
//       let newGear : GearSystem.GearPiece = {
//         gearId = oldGear.gearId;
//         name = oldGear.name;
//         description = oldGear.description;
//         slot = oldGear.slot;
//         rarity = oldGear.rarity;
//         category = oldGear.category;
//         terrainTag = oldGear.terrainTag;
//         ilvl = oldGear.ilvl;
//         season = oldGear.season;
//         speedBonus = oldGear.speedBonus;
//         powerCoreBonus = oldGear.powerCoreBonus;
//         accelerationBonus = oldGear.accelerationBonus;
//         stabilityBonus = oldGear.stabilityBonus;
//         luckBonus = oldGear.luckBonus;
//         passive = oldGear.passive;
//         boundToBot = 0; // Placeholder — admin_migrate_gear_soulbound() will wipe and regenerate
//         sourceRaceId = oldGear.sourceRaceId;
//         sourceEventType = oldGear.sourceEventType;
//         craftedFrom = oldGear.craftedFrom;
//         createdAt = oldGear.createdAt;
//       };
//       ignore Map.put(migratedGear, Map.nhash, gearId, newGear);
//       count += 1;
//     };
//
//     Debug.print("Gear migrated with boundToBot placeholder: " # Nat.toText(count) # " pieces");
//     { stable_gear_pieces = migratedGear };
//   }
// )
...
shared ({ caller = deployer }) persistent actor class McpServer(
  ...
  // Stable state for gear system
  let stable_gear_pieces = Map.new<Nat, GearSystem.GearPiece>(); // gearId -> GearPiece
  let stable_bot_loadouts = Map.new<Nat, GearSystem.BotLoadout>(); // tokenIndex -> BotLoadout
  let stable_player_gear_inventories = Map.new<Principal, GearSystem.PlayerGearInventory>(); // owner -> inventory
  let stable_consumable_instances = Map.new<Nat, GearSystem.ConsumableInstance>(); // instanceId -> ConsumableInstance
  var stable_next_gear_id : Nat = 1;
  var stable_next_consumable_id : Nat = 1;

  // Stable state for battery storage system
  let stable_garage_batteries = Map.new<Principal, PokedBotsGarage.GarageBatteryStorage>(); // owner -> battery storage
  let stable_bot_heat = Map.new<Nat, PokedBotsGarage.BotHeatStatus>(); // tokenIndex -> heat status
  var stable_next_battery_id : Nat = 1; // Global battery ID counter

  // Stable state for SMR (Small Modular Reactor) system
  let stable_user_smrs = Map.new<Principal, PokedBotsGarage.UserSMRStorage>(); // owner -> SMR storage
  ...
  private func reportTTError(errInfo : TT.ErrorReport) : ?Nat {
    ...
    let maxRetries = switch (action.actionType) {
      ...
      case ("event_finalization") { MAX_RETRIES_IMPORTANT };
  ...
  // Gear System Manager (gear/loot/crafting/consumables)
  transient let gearManager = GearSystem.GearManager(
    stable_gear_pieces,
    stable_bot_loadouts,
    stable_player_gear_inventories,
    stable_consumable_instances,
    {
      get = func() : Nat { stable_next_gear_id };
      set = func(id : Nat) { stable_next_gear_id := id };
    },
    {
      get = func() : Nat { stable_next_consumable_id };
      set = func(id : Nat) { stable_next_consumable_id := id };
    },
  );
  ...
  transient let garageManager = PokedBotsGarage.PokedBotsGarageManager(
    ...
    stable_garage_batteries,
    stable_bot_heat,
    stable_user_smrs,
    stable_user_repair_bays,
    {
      get = func() : Nat { stable_next_battery_id };
      set = func(id : Nat) { stable_next_battery_id := id };
    },
    {
      ...
      getGearBonuses = func(tokenIndex : Nat) : {
        speed : Nat;
        powerCore : Nat;
        acceleration : Nat;
        stability : Nat;
        ...
        gearManager.getGearStatBonuses(tokenIndex);
      };
    },
  );
        ...
        if (now < event.registrationCloses) {
          ...
          let getElo = func(tokenIndex : Nat) : Nat {
            switch (garageManager.getStats(tokenIndex)) {
              ...
              case (null) { 1500 }; // Default ELO for unknown bots
            };
          };
  ...
  func ensureCalendarScheduled<system>(now : Int) {
    ...
    // Get non-cancelled events for duplicate time checks
    // (Admin-cancelled events should not block rescheduled slots)
    let allEventsForTimeCheck = Array.filter<RaceCalendar.ScheduledEvent>(
      ...
      func(e) {
        ...
        e.scheduledTime >= now - NANOS_PER_DAY and e.scheduledTime <= now + (60 * NANOS_PER_DAY) and e.status != #Cancelled;
      },
    );
    ...
    if (weeklyLeagues.size() < 2) {
      ...
      // Get ALL weekly leagues (not just upcoming ones) to prevent duplicates
      let allWeeklyLeagues = Array.filter<RaceCalendar.ScheduledEvent>(
        allUpcomingEvents,
        func(e) {
          switch (e.eventType) {
            case (#WeeklyLeague) { true };
      ...
      for (i in Iter.range(0, 1 - weeklyLeagues.size())) {
        ...
        // FIX: Use allWeeklyLeagues (60 days) instead of upcomingEvents (14 days)
    ...
    // Schedule Daily Sprints to ensure at least 2 in next 48 hours (one per day at 18:00 UTC)
    if (sprintsIn48h.size() < 2) {
      // Always start from now to fill gaps in the 48-hour window
      var scheduleTime = now;
      var createdCount : Nat = 0;
      let targetCount : Nat = 2 - sprintsIn48h.size();

      // Limit iterations to prevent infinite loops (48h = 2 slots max)
      var iterations : Nat = 0;
      let maxIterations : Nat = 4; // Safety limit: check up to 4 slots
    ...
    // Rush Hour Rumble (every Friday 8pm MST = Saturday 3am UTC) - "Raid Night"
    ...
    // Champions Cup (last Sunday of each month at 10pm UTC)
        ...
        switch (raceManager.getRace(raceId)) {
          case (?race) {
              ...
              switch (entry.stats) {
                case (?stats) {
                  ...
                  let participant : RacingSimulator.RacingParticipant = {
                    ...
                    passives = gearManager.getEquippedPassives(tokenIdx);
                    consumables = gearManager.consumeLoadoutConsumables(tokenIdx);
                  };
            ...
            switch (raceSimulator.simulateRaceSegmented(updatedRace, participants)) {
              case (?(results, events, segmentBreakdown)) {
                ...
                // For multi-stage events (Rush Hour), per-race entryFee is 0 but event is paid
                let isFreeRace = if (race.entryFee == 0) {
                  switch (eventCalendar.getEventByRaceId(raceId)) {
                    case (?parentEvent) { parentEvent.metadata.entryFee == 0 };
                    case (null) { true };
                  };
                } else {
                  false;
  ...
  func handleRaceFinish<system>(actionId : TT.ActionId, action : TT.Action) : async* Star.Star<TT.ActionId, TT.Error> {
    Debug.print("[RF-1] Race finish handler triggered");
    ...
    switch (raceIdOpt) {
      case (?raceId) {
        Debug.print("[RF-2] Finishing race " # debug_show (raceId));
        ...
        switch (raceManager.getRace(raceId)) {
          case (?race) {
            ...
            Debug.print("[RF-3] About to mark race as Completed");
            ignore raceManager.updateRaceStatus(raceId, #Completed);
            Debug.print("[RF-4] Marked race as Completed");
            ...
            switch (race.results) {
              ...
              case (?results) {
                ...
                Debug.print("[RF-5] Applying race results, " # debug_show (results.size()) # " racers");
                ...
                // For multi-stage events (Rush Hour), per-race entryFee is 0 but event is paid
                let isFreeRaceForElo = if (race.entryFee == 0) {
                  switch (eventCalendar.getEventByRaceId(raceId)) {
                    case (?parentEvent) { parentEvent.metadata.entryFee == 0 };
                    case (null) { true };
                  };
                ...
                for (result in results.vals()) {
                  Debug.print("[RF-10] Processing result for " # result.nftId);
                  ...
                  Debug.print("[RF-11] recordRaceResult done for " # result.nftId);
                  ...
                  switch (Nat.fromText(result.nftId)) {
                    case (?tokenIdx) {
                      ...
                      Debug.print("[RF-12] dedication recorded for token " # Nat.toText(tokenIdx));
                    };
                    case (null) {};
                  };
                  ...
                  // Multi-stage events (Rush Hour etc.) DO award loot per race — each race is a "boss drop"
                  // For multi-stage Cumulative events, per-race entryFee is 0 but the EVENT has a fee
                  let isFreeRace = if (race.entryFee == 0) {
                    // Check if this race belongs to a paid event (multi-stage with aggregate prizes)
                    switch (eventCalendar.getEventByRaceId(raceId)) {
                      case (?parentEvent) { parentEvent.metadata.entryFee == 0 };
                      case (null) { true }; // Standalone race with 0 entry fee = free
                    };
                  } else {
                    false;
                  };
                  ...
                  if (not isFreeRace) {
                    ...
                    Debug.print("Skipping parts for free race (practice mode) - no part rewards");
                  };

                  // === GEAR LOOT DROP ===
                  // All races drop gear — free races drop Common/Standard (grey) gear via #Free tier
                  // Determine event tier from parent event (or default to Daily for standalone races)
                  let gearEventTier : GearSystem.EventTier = if (isFreeRace) {
                    #Free; // Free races always drop Common/Standard gear
                  } else {
                    switch (eventCalendar.getEventByRaceId(raceId)) {
                      case (?parentEvent) {
                        switch (parentEvent.eventType) {
                          case (#DailySprint) { #Daily };
                          case (#WeeklyLeague) { #Weekly };
                          case (#MonthlyCup) { #Monthly };
                          case (#SpecialEvent(_)) { #Special };
                        };
                      };
                      case (null) { #Daily }; // Standalone races use Daily tier
                    };
                  };

                  // Map race terrain to gear terrain tag
                  let gearTerrain : GearSystem.TerrainTag = switch (race.terrain) {
                    case (#MetalRoads) { #MetalRoads };
                    case (#ScrapHeaps) { #ScrapHeaps };
                    case (#WastelandSand) { #WastelandSand };
                  };

                  // Deterministic seed: raceId * 10000 + position * 137 + tokenIndex
                  let tokenIdx2 = switch (Nat.fromText(result.nftId)) {
                    case (?t) { t };
                    case (null) { 0 };
                  };
                  let lootSeed = raceId * 10000 + result.position * 137 + tokenIdx2;

                  // Current season and ilvl (derived from season — ilvl increases each season)
                  ...
                  switch (droppedGear) {
                    ...
                    case (null) {
                      Debug.print("No gear drop (miss) for " # Principal.toText(result.owner));
                    };
                  };
                ...
                Debug.print("[RF-15] Race completed successfully - " # debug_show (results.size()) # " participants updated");

                // Chain to next race in event if there is one
                Debug.print("[RF-16] Checking for event chaining");
                switch (eventCalendar.getEventByRaceId(raceId)) {
                  case (?event) {
                    // Find current race index in event
                    let raceIndexOpt = Array.indexOf<Nat>(raceId, event.raceIds, Nat.equal);
                    switch (raceIndexOpt) {
                      case (?currentIndex) {
                        ...
                        if (currentIndex + 1 < event.raceIds.size()) {
                          ...
                          // Schedule event finalization as a separate action for better error isolation
                          let eventFinalizeActionId = tt().setActionASync<system>(
                            Int.abs(Time.now() + 5_000_000_000), // 5 seconds delay
                            {
                              actionType = "event_finalization";
                              params = to_candid (event.eventId);
                            },
                            60_000_000_000, // 60 second timeout
                          );
                          Debug.print("Scheduled event_finalization action " # debug_show (eventFinalizeActionId) # " for event " # Nat.toText(event.eventId));
                        };
                      };
                      case (null) {
  ...
  // Handle event finalization (multi-stage scoring)
  func handleEventFinalization<system>(actionId : TT.ActionId, action : TT.Action) : async* Star.Star<TT.ActionId, TT.Error> {
    Debug.print("=== EVENT FINALIZATION HANDLER STARTED ===");

    let eventIdOpt : ?Nat = from_candid (action.params);

    switch (eventIdOpt) {
      case (?eventId) {
        Debug.print("[STEP 0] Finalizing event " # Nat.toText(eventId));

        switch (eventCalendar.getEvent(eventId)) {
          case (?event) {
            Debug.print("[STEP 1] Event found: " # event.metadata.name);
            Debug.print("[STEP 2] Scoring mode: " # debug_show (event.metadata.scoringMode));
            Debug.print("[STEP 3] Race IDs: " # debug_show (event.raceIds));
            Debug.print("[STEP 4] Registrations count: " # Nat.toText(event.registrations.size()));
            ...
            switch (event.metadata.scoringMode) {
              case (#TeamAggregate) {
                ...
                Debug.print("[TA-1] Processing TeamAggregate scoring for Faction Wars event " # Nat.toText(event.eventId));
                ...
                Debug.print("[TA-2] Iterating through " # Nat.toText(event.raceIds.size()) # " races");
                for (eventRaceId in event.raceIds.vals()) {
                  Debug.print("[TA-2a] Processing race " # Nat.toText(eventRaceId));
                  switch (raceManager.getRace(eventRaceId)) {
                    case (?eventRace) {
                      Debug.print("[TA-2b] Race found, checking results");
                      switch (eventRace.results) {
                        case (?raceResults) {
                          Debug.print("[TA-2c] Found " # Nat.toText(raceResults.size()) # " results");
                          for (result in raceResults.vals()) {
                            ...
                            switch (Nat.fromText(result.nftId)) {
                              case (?tokenIndex) {
                                switch (garageManager.getStats(tokenIndex)) {
                                  case (?botStats) {
                                    // Convert faction to text key - handle ALL faction variants
                                  ...
                                  case (null) {
                                    Debug.print("[TA-2d] No stats for token " # result.nftId);
                                  };
                                };
                              };
                              case (null) {
                                Debug.print("[TA-2e] Could not parse nftId: " # result.nftId);
                              };
                            };
                          };
                        };
                        case (null) {
                          Debug.print("[TA-2f] No results for race " # Nat.toText(eventRaceId));
                        };
                      };
                    };
                    case (null) {
                      Debug.print("[TA-2g] Race not found: " # Nat.toText(eventRaceId));
                    };
                  };
                };

                // Find winning faction
                Debug.print("[TA-3] Finding winning faction");
                ...
                Debug.print("[TA-4] Winning faction: " # winningFaction # " with " # Nat.toText(winningPoints) # " points");

                // Calculate total prize pool from entry fees paid by all registrants
                Debug.print("[TA-5] Calculating prize pool");
                var totalPrizePool : Nat = event.metadata.prizePoolBonus;
                Debug.print("[TA-5a] Starting with prizePoolBonus: " # Nat.toText(event.metadata.prizePoolBonus));
                ...
                Debug.print("[TA-5b] After entry fees: " # Nat.toText(totalPrizePool));

                // Add event bonus prize
                totalPrizePool += event.metadata.eventBonusPrize;
                Debug.print("[TA-5c] After eventBonusPrize: " # Nat.toText(totalPrizePool));

                // Apply 5% platform tax
                let platformTax = (totalPrizePool * 5) / 100;
                Debug.print("[TA-5d] Platform tax: " # Nat.toText(platformTax));
                let netPrizePool = if (totalPrizePool >= platformTax) {
                  totalPrizePool - platformTax;
                } else { 0 };
                Debug.print("[TA-5e] Net prize pool: " # Nat.toText(netPrizePool));

                // Distribute FULL prize pool to winning faction members (split equally)
                Debug.print("[TA-6] Distributing prizes");
                  ...
                  switch (Map.get(factionMembers, Map.thash, winningFaction)) {
                    case (?winners) {
                      if (winners.size() > 0) {
                        ...
                        Debug.print("[TA-6a] Prize per member: " # Nat.toText(prizePerMember) # " to " # Nat.toText(winners.size()) # " members");

                        var memberIndex : Nat = 0;
                        for (winner in winners.vals()) {
                          ignore tt().setActionASync<system>(
                            Int.abs(Time.now() + (10_000_000_000 * (memberIndex + 1))),
                            {
                              actionType = "prize_distribution";
                              params = to_candid ({
                                raceId = 0 : Nat;
                    ...
                    case (null) {
                      Debug.print("[TA-6b] No winners found for faction: " # winningFaction);
                    };
                  };
                };

                // Award parts to winning faction members
                Debug.print("[TA-7] Awarding parts to winning faction members");
                switch (Map.get(factionMembers, Map.thash, winningFaction)) {
                  case (?winners) {
                    let partsPerMember : Nat = 38;
                    for (winner in winners.vals()) {
                      garageManager.addParts(winner, #UniversalPart, partsPerMember);
                      Debug.print("[TA-7a] Awarded " # Nat.toText(partsPerMember) # " UniversalParts to " # Principal.toText(winner));
                    };
                  };
                  case (null) {};
                };
                Debug.print("[TA-8] TeamAggregate processing complete");
              };

              case (#Cumulative) {
                // MULTI-STAGE EVENT: Sum points per bot, distribute FULL prize pool based on cumulative standings
                Debug.print("[CUM-1] Processing Cumulative scoring for event " # Nat.toText(event.eventId));
                        ...
                        case (?raceResults) {
                          for (result in raceResults.vals()) {
                            let positionPoints : Nat = if (result.position == 1) {
                              ...
                              4;
                            } else if (result.position == 4) { 2 } else { 1 };
                ...
                Debug.print("[CUM-2] Prize pool: " # Nat.toText(totalPrizePool) # " (net: " # Nat.toText(netPrizePool) # ")");
            ...
            Debug.print("[FINAL] Marking event as completed...");
            ignore eventCalendar.updateEventStatus(event.eventId, #Completed);
            Debug.print("[FINAL] Event " # Nat.toText(event.eventId) # " marked as Completed");
          };
          case (null) {
            Debug.print("Event not found: " # Nat.toText(eventId));
          };
        };
      };
      case (null) {
        Debug.print("Could not decode event ID from params");
      };
    };

    Debug.print("=== EVENT FINALIZATION HANDLER COMPLETED ===");
    return #awaited(actionId);
  };
  ...
  tt().registerExecutionListenerAsync(?"event_finalization", handleEventFinalization);
  ...
  transient let tools : [McpTypes.Tool] = [
    ...
    GarageCombineParts.config(),
    // Battery management tools
    GarageListBatteries.config(),
    GarageJoltBot.config(),
    GarageRepairBattery.config(),
    GarageRebuildBattery.config(),
    GarageSalvageBattery.config(),
    // Station status tool
    GarageGetStationStatus.config(),
    // Activity alias for scavenging
    GarageStartActivity.config(),
    // Bulk bot details
    GarageGetBulkDetails.config(),
    // Complete all ready scavenging
    GarageCompleteAllReadyScavenging.config(),
  ...
  transient let mcpConfig : McpTypes.McpConfig = {
    ...
    toolImplementations = [
      ...
      ("garage_combine_parts", GarageCombineParts.handle(toolContext)),
      // Battery management tools
      ("garage_list_batteries", GarageListBatteries.handle(toolContext)),
      ("garage_jolt_bot", GarageJoltBot.handle(toolContext)),
      ("garage_repair_battery", GarageRepairBattery.handle(toolContext)),
      ("garage_rebuild_battery", GarageRebuildBattery.handle(toolContext)),
      ("garage_salvage_battery", GarageSalvageBattery.handle(toolContext)),
      // Station status tool
      ("garage_get_station_status", GarageGetStationStatus.handle(toolContext)),
      // Activity alias for scavenging
      ("garage_start_activity", GarageStartActivity.handle(toolContext)),
      // Bulk bot details
      ("garage_get_bulk_details", GarageGetBulkDetails.handle(toolContext)),
      // Complete all ready scavenging
      ("garage_complete_all_ready_scavenging", GarageCompleteAllReadyScavenging.handle(toolContext)),
  ...
  public shared func debug_test_simulation(
    ...
    switch (raceSimulator.simulateRaceSegmented(testRace, participants)) {
      case (?(results, events, _segmentBreakdown)) {
  ...
  public shared ({ caller }) func debug_resimulate_race(raceId : Nat) : async ?{
    ...
    switch (raceSimulator.simulateRaceSegmented(race, participants)) {
      case (?(newResults, events, _segmentBreakdown)) {
  ...
  public shared func debug_simulate_race(
    ...
    switch (simulator.simulateRaceSegmented(mockRace, participants)) {
      case (?(results, raceEvents, _segmentBreakdown)) {
        // Calculate analysis
  ...
  public shared ({ caller }) func create_user_event(
    config : RaceCalendar.UserEventConfig,
  ) : async Result.Result<{ eventId : Nat; message : Text }, Text> {
    trackMethodCall("create_user_event", caller);

    let now = Time.now();

    // Validate the event config via RaceCalendar
    let event = switch (eventCalendar.scheduleUserEvent(config, caller, now)) {
      case (#err(e)) { return #err(e) };
      case (#ok(e)) { e };
    };

    // Calculate total payment: creation fee + prize contribution
    let totalPayment = RaceCalendar.USER_EVENT_CREATION_FEE + config.prizeContribution;
    ...
    let transferResult = try {
      ...
      // Rollback: cancel the event we just created since payment failed
      ignore eventCalendar.cancelUserEvent(event.eventId, caller, now);
      return #err("Payment transfer failed: " # Error.message(e));
    };

    switch (transferResult) {
      case (#Err(e)) {
        // Rollback: cancel the event since payment failed
        ignore eventCalendar.cancelUserEvent(event.eventId, caller, now);
        let errorMsg = switch (e) {
          case (#InsufficientAllowance({ allowance })) {
            "Insufficient spending allowance. Current: " # Nat.toText(allowance / 100_000_000) # " ICP. Required: " # Nat.toText(totalPayment / 100_000_000) # " ICP. Please approve more ICP first.";
          };
          case (#InsufficientFunds({ balance })) {
            "Insufficient ICP balance. Current: " # Nat.toText(balance / 100_000_000) # " ICP. Required: " # Nat.toText(totalPayment / 100_000_000) # " ICP.";
          };
          case (_) {
      ...
      case (#Ok(_blockIndex)) {
        // Payment successful — event is already created
        let visStr = switch (config.visibility) {
          case (#Public) { "Public" };
          case (#Private) { "Private (invite-only)" };
          case (#Restricted(_)) { "Restricted (whitelist)" };
        };

        #ok({
          eventId = event.eventId;
          message = "🏁 Event \"" # config.name # "\" created! Visibility: " # visStr #
          ". Creation fee: " # Nat.toText(RaceCalendar.USER_EVENT_CREATION_FEE / 100_000_000) # " ICP" #
          ". Prize pool contribution: " # Nat.toText(config.prizeContribution / 100_000_000) # " ICP (escrowed).";
        });
      };
    };
  };
  ...
  public shared ({ caller }) func cancel_user_event(
    eventId : Nat,
  ) : async Result.Result<{ message : Text; refundedRegistrants : Nat; prizeRefunded : Nat }, Text> {
    trackMethodCall("cancel_user_event", caller);

    let now = Time.now();

    // Cancel the event via RaceCalendar (validates ownership, status, etc.)
    let cancelledEvent = switch (eventCalendar.cancelUserEvent(eventId, caller, now)) {
      case (#err(e)) { return #err(e) };
      case (#ok(e)) { e };
    };
    ...
    // Refund entry fees to all registrants
    if (cancelledEvent.registrations.size() > 0) {
      let ledgerId = switch (icpLedgerCanisterId) {
        case (?id) { id };
        case (null) { return #err("ICP Ledger not configured — cannot process refunds") };
      };
      ...
      for (reg in cancelledEvent.registrations.vals()) {
        try {
          let refundResult = await icpLedger.icrc1_transfer({
            to = { owner = reg.owner; subaccount = null };
          ...
          switch (refundResult) {
            case (#Ok(_)) { refundedCount += 1 };
            case (#Err(_)) {};
          };
        } catch (_) {
          // Log but continue — don't fail the whole cancellation for one refund
        };
      };
    };

    // Refund prize contribution to creator
    var prizeRefunded : Nat = 0;
    if (cancelledEvent.metadata.prizePoolBonus > 0) {
      let ledgerId = switch (icpLedgerCanisterId) {
        case (?id) { id };
        case (null) { return #err("Refunds processed but prize return failed: no ledger") };
      };
      ...
      try {
        let prizeRefundResult = await icpLedger.icrc1_transfer({
          to = { owner = caller; subaccount = null };
          amount = cancelledEvent.metadata.prizePoolBonus;
          fee = null;
          memo = null;
          created_at_time = null;
          from_subaccount = null;
        });
        switch (prizeRefundResult) {
          case (#Ok(_)) { prizeRefunded := cancelledEvent.metadata.prizePoolBonus };
          case (#Err(_)) {};
        };
      } catch (_) {};
    };
    ...
    #ok({
      message = "🚫 Event \"" # cancelledEvent.metadata.name # "\" cancelled." #
      (if (refundedCount > 0) {
        " Refunded " # Nat.toText(refundedCount) # " registrant(s)."
      } else { "" }) #
      (if (prizeRefunded > 0) {
        " Prize contribution of " # Nat.toText(prizeRefunded / 100_000_000) # " ICP returned."
      } else { "" }) #
      " Note: creation fee is non-refundable.";
      refundedRegistrants = refundedCount;
      prizeRefunded = prizeRefunded;
    });
  };

  /// Get events created by the calling user
  public query ({ caller }) func get_my_events() : async [RaceCalendar.ScheduledEvent] {
    eventCalendar.getUserEvents(caller);
  };
  ...
  // Deterministic replay: re-simulate a completed race and return per-bot segment times
  public query func get_race_segments(raceId : Nat) : async Result.Result<[RacingSimulator.BotSegmentTimes], Text> {
    let ?race = Map.get(stable_races, Map.nhash, raceId) else {
      return #err("Race not found");
    };

    let ?results = race.results else {
      return #err("Race has no results");
    };

    // Rebuild participants from entries (preserving original order for deterministic seeds)
    // The simulation seed uses participant index, so order must match the original simulation
    let participants = Array.map<RacingSimulator.RaceEntry, RacingSimulator.RacingParticipant>(
      ...
      func(entry) : RacingSimulator.RacingParticipant {
        ...
        // Look up stored result for this entry to get gear data (faction/passives/consumables)
        let matchingResult = Array.find<RacingSimulator.RaceResult>(results, func(r) { r.nftId == entry.nftId });
        let (faction, passives, consumables) = switch (matchingResult) {
          case (?r) { (r.faction, r.passives, r.consumables) };
          case (null) {
            (#Wild, ([] : [GearSystem.PassiveEffect]), ([] : [GearSystem.ConsumableInstance]));
          };
        };
        {
          nftId = entry.nftId;
          owner = entry.owner;
          stats = Option.get(entry.stats, { speed = 50; powerCore = 50; acceleration = 50; stability = 50; luck = 10; overcharge = 0; perfectTuneUp = false; tuneupQuality = 0.0; baseAvgRating = null });
          tokenIndex = tokenIdx;
          faction = faction;
          baseAvgRating = switch (entry.stats) {
            case (?s) { s.baseAvgRating };
            case (null) { null };
          };
          passives = passives;
  ...
  public shared ({ caller }) func debug_regenerate_race_commentary(raceId : Nat) : async Result.Result<Text, Text> {
    ...
    let ?(_, newEvents, _) = raceSimulator.simulateRaceSegmented(race, participants) else {
  ...
  // Admin: Wipe all gear and regenerate from race history with soulbound bindings.
  // This replays every completed paid race's loot drops, binding each gear piece to the bot that earned it.
  // Run once after deploying soulbound gear changes.
  public shared ({ caller }) func admin_migrate_gear_soulbound() : async Result.Result<Text, Text> {
    if (caller != owner) {
      return #err("Unauthorized: Only owner can run gear migration");
    };

    // Build the race descriptor array for wipeAndRegenerateGear
    let allRaces = raceManager.getAllRaces();
    let raceBuf = Buffer.Buffer<{ raceId : Nat; terrain : GearSystem.TerrainTag; entryFee : Nat; results : ?[{ nftId : Text; owner : Principal; position : Nat }]; startTime : Int; eventTier : ?GearSystem.EventTier; isPartOfPaidEvent : Bool }>(allRaces.size());
    ...
    for (race in allRaces.vals()) {
      ...
      switch (race.status) {
        case (#Completed) {
          ...
          // Determine event tier from parent event
          let eventTier : ?GearSystem.EventTier = switch (eventCalendar.getEventByRaceId(race.raceId)) {
            case (?parentEvent) {
              let tier : GearSystem.EventTier = switch (parentEvent.eventType) {
                case (#DailySprint) { #Daily };
                case (#WeeklyLeague) { #Weekly };
                case (#MonthlyCup) { #Monthly };
                case (#SpecialEvent(_)) { #Special };
          ...
          // Check if race is part of a paid event (for free-race filtering)
          let isPartOfPaidEvent = if (race.entryFee == 0) {
            switch (eventCalendar.getEventByRaceId(race.raceId)) {
              case (?parentEvent) { parentEvent.metadata.entryFee > 0 };
              case (null) { false };
            };
          } else { false };

          // Map results to minimal shape
          let results : ?[{ nftId : Text; owner : Principal; position : Nat }] = switch (race.results) {
            case (?rs) {
              let buf = Buffer.Buffer<{ nftId : Text; owner : Principal; position : Nat }>(rs.size());
              for (r in rs.vals()) {
                buf.add({
                  nftId = r.nftId;
                  owner = r.owner;
          ...
          raceBuf.add({
            ...
            startTime = race.startTime;
            eventTier = eventTier;
            isPartOfPaidEvent = isPartOfPaidEvent;
          });
        };
        case (_) {}; // Skip non-completed races
      };
    };

    let raceData = Buffer.toArray(raceBuf);
    // March 12, 2026 00:00:00 UTC in nanoseconds — gear system deploy date
    let gearSystemDeployTime : Int = 1_773_273_600_000_000_000;
    let (gearsGenerated, racesProcessed) = gearManager.wipeAndRegenerateGear(
      raceData,
      Leaderboard.getSeasonIdFromTime,
      gearSystemDeployTime,
    );

    let msg = "Gear migration complete: " # Nat.toText(gearsGenerated) # " gear pieces generated from " # Nat.toText(racesProcessed) # " races (all soulbound)";
    Debug.print(msg);
    #ok(msg);
  };
  ...
  public shared ({ caller }) func admin_resimulate_race(raceId : Nat) : async Result.Result<Text, Text> {
    ...
    let ?(newResults, newEvents, _) = raceSimulator.simulateRaceSegmented(race, participants) else {
    ...
    let updatedResults = Array.tabulate<RacingSimulator.RaceResult>(
      ...
      func(i) : RacingSimulator.RaceResult {
        ...
        };
        {
          ...
          dnf = newResult.dnf;
          faction = newResult.faction;
          passives = newResult.passives;
          consumables = newResult.consumables;
        };
      },
    );
  ...
  public shared ({ caller }) func force_finalize_event(eventId : Nat) : async Text {
    if (caller != owner) {
      return "Unauthorized: only owner can force-finalize events";
    };

    // Trigger the event finalization handler directly and await completion
    let dummyActionId : TT.ActionId = { id = 0; time = Int.abs(Time.now()) };
    let result = await* handleEventFinalization<system>(
      dummyActionId,
      {
        actionType = "event_finalization";
        params = to_candid (eventId);
        retries = 0;
        aSync = ?60_000_000_000; // 60 second timeout
      },
    );
    ...
    switch (result) {
      case (#awaited(_)) {
        "Force-finalized event " # Nat.toText(eventId) # " successfully";
      };
      case (#trappable(_)) {
        "Force-finalize triggered for event " # Nat.toText(eventId) # " but encountered an error";
      };
      case (_) {
        "Force-finalize triggered for event " # Nat.toText(eventId) # " with unknown result";
      };
    };
  };
  ...
  // Admin function to grant a battery to a user for testing
  public shared ({ caller }) func admin_grant_battery(targetPrincipal : Principal, batteryType : Text) : async Text {
    if (caller != owner) {
      return "Unauthorized: only owner can grant batteries";
    };

    let now = Time.now();
    let bType : PokedBotsGarage.BatteryType = switch (batteryType) {
      case ("ScrapCell") { #ScrapCell };
      case ("SalvagePack") { #SalvagePack };
      case ("IndustrialBank") { #IndustrialBank };
      case ("PlasmaVault") { #PlasmaVault };
      case (_) {
        return "Invalid battery type. Use: ScrapCell, SalvagePack, IndustrialBank, or PlasmaVault";
      };
    };

    // Create a fresh battery (100% health, 0 cycles, ready to charge)
    let battery : PokedBotsGarage.Battery = {
      id = garageManager.getNextBatteryId();
      batteryType = bType;
    ...
    garageManager.addBatteryToGarage(targetPrincipal, battery);
    "Granted " # batteryType # " (ID: " # Nat.toText(battery.id) # ") to " # Principal.toText(targetPrincipal);
  };

  // Admin function to remove a battery from a user's garage
  public shared ({ caller }) func admin_remove_battery(targetPrincipal : Principal, batteryId : Nat) : async Text {
    if (caller != owner) {
      return "Unauthorized: only owner can remove batteries";
    };

    switch (garageManager.removeBatteryFromGarage(targetPrincipal, batteryId)) {
      case (null) {
        "Battery ID " # Nat.toText(batteryId) # " not found in garage of " # Principal.toText(targetPrincipal);
      };
      case (?battery) {
        ...
        "Removed " # typeText # " (ID: " # Nat.toText(batteryId) # ") from " # Principal.toText(targetPrincipal);
      };
    };
  };

  // Admin function to reset all ELOs to 1500
  public shared ({ caller }) func admin_reset_elos() : async Text {
    if (caller != owner) {
      return "Unauthorized: Only owner can reset ELOs";
    };

    // Reset all initialized bots to ELO 1500
    let allTokenIndices = garageManager.getAllInitializedTokenIndices();
    var botsReset = 0;

    for (tokenIndex in allTokenIndices.vals()) {
      switch (garageManager.getStats(tokenIndex)) {
        case (?botStats) {
          let resetStats : PokedBotsGarage.PokedBotRacingStats = {
            botStats with
            eloRating = 1500; // Reset to neutral mid-point
          };
          garageManager.updateStats(tokenIndex, resetStats);
          botsReset += 1;
        };
        case (null) {};
      };
    ...
    "ELO Reset Complete: " # Nat.toText(botsReset) # " bots reset to 1500 ELO";
  };
  ...
  // Admin function to force-trigger the calendar scheduler
  public shared ({ caller }) func admin_trigger_scheduler() : async Text {
    if (caller != owner) {
      Debug.trap("Only owner can trigger scheduler");
    };
    let now = Time.now();
    ensureCalendarScheduled<system>(now);
    "Scheduler triggered at " # Int.toText(now);
  };

  // Admin function to add Scrap race stages to a Rush Hour event's Manual templates
  public shared ({ caller }) func admin_add_scrap_to_rush_hour(eventId : Nat) : async Text {
    if (caller != owner) {
      Debug.trap("Only owner can modify events");
    };

    let scrapTemplates : [RaceCalendar.RaceTemplate] = [
      {
        stageName = ?"Race 1";
        raceClass = #Scrap;
        terrain = #MetalRoads;
    ...
    switch (eventCalendar.getEvent(eventId)) {
      case (?event) {
        switch (event.raceCreationMode) {
          case (#Manual(config)) {
            let newTemplates = Array.append(scrapTemplates, config.raceTemplates);
            let newMode : RaceCalendar.RaceCreationMode = #Manual({
              raceTemplates = newTemplates;
              heatAllocation = config.heatAllocation;
            });
            switch (eventCalendar.updateEventRaceCreationMode(eventId, newMode)) {
              case (?updated) {
                "Added 5 Scrap race stages to event " # Nat.toText(eventId) # " (" # updated.metadata.name # "). Now has " # Nat.toText(newTemplates.size()) # " total race templates.";
              };
              case (null) {
                "Error: Failed to update event " # Nat.toText(eventId);
              };
            };
          };
          case (#Automatic(_)) {
            "Error: Event " # Nat.toText(eventId) # " uses Automatic mode, not Manual. Scrap will work automatically via divisions.";
          };
        };
      };
      case (null) { "Error: Event " # Nat.toText(eventId) # " not found" };
    };
  };

  // Admin function to update event metadata (divisions, minEntries, etc.)
  public shared ({ caller }) func admin_update_event_metadata(
    eventId : Nat,
    divisions : [Text],
    minEntries : Nat,
  ) : async Text {
    if (caller != owner) {
      Debug.trap("Only owner can update event metadata");
    };

    // Parse division texts to RaceClass variants
    let parsedDivisions = Buffer.Buffer<RaceCalendar.RaceClass>(divisions.size());
    for (div in divisions.vals()) {
      switch (div) {
        case ("Scrap") { parsedDivisions.add(#Scrap) };
        case ("Junker") { parsedDivisions.add(#Junker) };
        case ("Raider") { parsedDivisions.add(#Raider) };
        case ("Elite") { parsedDivisions.add(#Elite) };
        case ("SilentKlan") { parsedDivisions.add(#SilentKlan) };
        case (other) {
          return "Error: Unknown division '" # other # "'. Valid: Scrap, Junker, Raider, Elite, SilentKlan";
        };
      };
    };
    ...
    switch (eventCalendar.getEvent(eventId)) {
      case (?event) {
        let updatedMetadata : RaceCalendar.EventMetadata = {
          event.metadata with
          divisions = Buffer.toArray(parsedDivisions);
          minEntries = minEntries;
        };
        switch (eventCalendar.updateEventMetadata(eventId, updatedMetadata)) {
          case (?updated) {
            "Updated event " # Nat.toText(eventId) # " (" # updated.metadata.name # "): divisions=" # debug_show (Buffer.toArray(parsedDivisions)) # ", minEntries=" # Nat.toText(minEntries);
          };
          case (null) {
            "Error: Event " # Nat.toText(eventId) # " not found";
          };
  ...
  public shared ({ caller }) func trigger_race_finish(raceId : Nat) : async Text {
    ...
    switch (raceManager.getRace(raceId)) {
      case (?race) {
        ...
        let actionId : TT.ActionId = { id = 0; time = Int.abs(Time.now()) };
        let action : TT.Action = {
          actionType = "race_finish";
          params = to_candid (raceId);
          aSync = null;
        ...
        Debug.print("[TRIGGER-RF] About to call handleRaceFinish for race " # Nat.toText(raceId));
        let result = await* handleRaceFinish<system>(actionId, action);

        switch (result) {
          case (#awaited(_)) {
            Debug.print("[TRIGGER-RF] handleRaceFinish completed successfully (awaited)");
            "Race " # Nat.toText(raceId) # " finished successfully";
          };
          case (#trappable(err)) {
            Debug.print("[TRIGGER-RF] handleRaceFinish error (trappable): " # debug_show (err));
            "Race " # Nat.toText(raceId) # " failed: " # debug_show (err);
          };
          case (#err(e)) {
            Debug.print("[TRIGGER-RF] handleRaceFinish error: " # debug_show (e));
            "Race " # Nat.toText(raceId) # " error: " # debug_show (e);
          };
        };
      };
      case (null) {
  ...
  public shared query ({ caller }) func web_get_garage_power_status() : async {
    ...
    // SMR info
    smrCapacityWatts : Nat;
    installedSMRCount : Nat;
    // Battery charging info
    surplusWatts : Nat;
    batteriesCharging : Nat;
    ...
    } else { 0.0 };
    {
      ...
      smrCapacityWatts = smrStorage.totalPowerOutput;
      installedSMRCount = smrStorage.installedSMRs.size();
  ...
  public shared query ({ caller }) func web_get_user_smrs() : async {
    installedSMRs : [{
      model : Text;
      powerOutput : Nat;
      installedAt : Int;
    ...
    {
      installedSMRs = smrsWithLifetime;
      totalPowerOutput = smrStorage.totalPowerOutput;
    };
  };
  ...
  public shared ({ caller }) func web_purchase_smr(
    ...
    let model : PokedBotsGarage.SMRModel = switch (modelId) {
      case ("WR250") { #WR250 };
      case ("WR-250") { #WR250 };
      case ("smr-basic") { #WR250 };
      case ("WR500") { #WR500 };
      case ("WR-500") { #WR500 };
      case ("smr-standard") { #WR500 };
      case ("WR880") { #WR880 };
      case ("WR-880") { #WR880 };
      case ("smr-advanced") { #WR880 };
      case ("WR1210") { #WR1210 };
      case ("WR-1210") { #WR1210 };
      case ("smr-premium") { #WR1210 };
      case (_) {
        return #err("Invalid SMR model. Valid models: WR250, WR500, WR880, WR1210");
      };
    };

    let icpCost = PokedBotsGarage.getSMRIcpCost(model);
    let powerOutput = PokedBotsGarage.getSMRPowerOutput(model);
    let modelName = PokedBotsGarage.getSMRModelName(model);
    ...
    switch (transferResult) {
      ...
      case (#Ok(_blockIndex)) {
        // Payment successful, install the SMR
        let now = Time.now();
        let _installedSMR = garageManager.installSMR(caller, model, now);

        // Get new total capacity
        let newStatus = garageManager.getGaragePowerStatus(caller);

        #ok({
          message = "☢️ " # modelName # " installed! Your garage now has +" # Nat.toText(powerOutput) # " MW additional power capacity.";
          model = modelName;
  ...
  public shared query ({ caller }) func web_get_user_repair_bays() : async {
    bays : [{
      bayId : Nat;
      tier : Nat;
      tierName : Text;
      repairRatePerHour : Nat;
      powerDrawWatts : Nat;
      currentBotToken : ?Nat;
      repairStartTime : ?Int;
      repairStartCondition : ?Nat;
      upgradeInProgress : ?{
        targetTier : Nat;
        targetTierName : Text;
        startTime : Int;
    ...
    totalBays : Nat;
    maxBays : Nat;
    totalPartsInvested : Nat;
    totalIcpInvested : Nat;
    nextSlotCost : ?{ parts : Nat; icpE8s : Nat };
    fallbackRepairRate : Nat;
  } {
    let storage = garageManager.getUserRepairBayStorage(caller);
    let now = Time.now();

    let baysWithDetails = Array.map<PokedBotsGarage.RepairBay, { bayId : Nat; tier : Nat; tierName : Text; repairRatePerHour : Nat; powerDrawWatts : Nat; currentBotToken : ?Nat; repairStartTime : ?Int; repairStartCondition : ?Nat; upgradeInProgress : ?{ targetTier : Nat; targetTierName : Text; startTime : Int; completionTime : Int; remainingSeconds : Int } }>(
      storage.bays,
      func(bay : PokedBotsGarage.RepairBay) : {
        bayId : Nat;
        tier : Nat;
        tierName : Text;
        repairRatePerHour : Nat;
        ...
        let tierConfig = switch (PokedBotsGarage.getRepairBayTierConfig(bay.tier)) {
          case (?c) { c };
          case (null) { PokedBotsGarage.REPAIR_BAY_TIERS[0] };
        };

        let upgradeInfo = switch (bay.upgradeInProgress) {
          case (?u) {
            let targetConfig = switch (PokedBotsGarage.getRepairBayTierConfig(u.targetTier)) {
              case (?c) { c };
              case (null) { PokedBotsGarage.REPAIR_BAY_TIERS[0] };
            };
            let remaining = if (u.completionTime > now) {
              (u.completionTime - now) / 1_000_000_000;
            } else { 0 };
            ?{
              targetTier = u.targetTier;
              targetTierName = targetConfig.name;
...

        {
          ...
          repairRatePerHour = tierConfig.repairRatePerHour;
          powerDrawWatts = tierConfig.powerDrawWatts;
          currentBotToken = bay.currentBotToken;
          repairStartTime = bay.repairStartTime;
          repairStartCondition = bay.repairStartCondition;
    ...
    let nextCost = if (nextSlot <= PokedBotsGarage.MAX_REPAIR_BAYS) {
      switch (PokedBotsGarage.getRepairBaySlotCost(nextSlot)) {
        case (?(parts, icp)) { ?{ parts = parts; icpE8s = icp } };
        case (null) { null };
      };
    } else { null };

    {
      bays = baysWithDetails;
      totalBays = storage.bays.size();
      maxBays = PokedBotsGarage.MAX_REPAIR_BAYS;
      totalPartsInvested = storage.totalPartsInvested;
      totalIcpInvested = storage.totalIcpInvested;
      nextSlotCost = nextCost;
      fallbackRepairRate = PokedBotsGarage.FALLBACK_REPAIR_RATE;
    };
  };

  /// Get upgrade costs for a specific repair bay
  public shared query ({ caller }) func web_get_repair_bay_upgrade_cost(
    bayId : Nat
  ) : async Result.Result<{ currentTier : Nat; currentTierName : Text; nextTier : Nat; nextTierName : Text; partsCost : Nat; icpCostE8s : Nat; buildTimeSeconds : Int; newRepairRate : Nat; newPowerDraw : Nat }, Text> {
    let storage = garageManager.getUserRepairBayStorage(caller);

    var foundBay : ?PokedBotsGarage.RepairBay = null;
    for (bay in storage.bays.vals()) {
      if (bay.bayId == bayId) { foundBay := ?bay };
    };

    switch (foundBay) {
      case (null) { #err("Bay not found: " # Nat.toText(bayId)) };
      case (?bay) {
        if (bay.tier >= 16) {
          return #err("Bay is already at maximum tier (16)");
        };

        let currentConfig = switch (PokedBotsGarage.getRepairBayTierConfig(bay.tier)) {
          case (?c) { c };
          case (null) { return #err("Invalid current tier") };
        };

        let nextTier = bay.tier + 1;
        let nextConfig = switch (PokedBotsGarage.getRepairBayTierConfig(nextTier)) {
          case (?c) { c };
          case (null) { return #err("Invalid next tier") };
        };
        ...
        #ok({
          ...
          partsCost = nextConfig.partsCost;
          icpCostE8s = nextConfig.icpCostE8s;
          buildTimeSeconds = nextConfig.buildTimeNanos / 1_000_000_000;
          newRepairRate = nextConfig.repairRatePerHour;
          newPowerDraw = nextConfig.powerDrawWatts;
        });
      };
    };
  };
  ...
  public shared ({ caller }) func web_purchase_repair_bay_slot() : async Result.Result<{ message : Text; bayId : Nat; totalBays : Nat }, Text> {
    ...
    if (nextSlot > PokedBotsGarage.MAX_REPAIR_BAYS) {
      return #err("Already at maximum repair bays (" # Nat.toText(PokedBotsGarage.MAX_REPAIR_BAYS) # ")");
    };

    let (requiredParts, requiredIcp) = switch (PokedBotsGarage.getRepairBaySlotCost(nextSlot)) {
      case (?(p, i)) { (p, i) };
      case (null) { return #err("Invalid slot configuration") };
    };

    // Check user has enough parts
    let inventory = garageManager.getUserInventory(caller);
    let totalParts = inventory.speedChips + inventory.powerCoreFragments + inventory.thrusterKits + inventory.gyroModules + inventory.universalParts;
    if (totalParts < requiredParts) {
      return #err("Insufficient parts: need " # Nat.toText(requiredParts) # ", have " # Nat.toText(totalParts));
    };

    // Process ICP payment if required
    if (requiredIcp > 0) {
      let ledgerId = switch (icpLedgerCanisterId) {
        case (?id) { id };
        case (null) { return #err("ICP Ledger not configured") };
      ...
      switch (transferResult) {
        case (#Err(e)) {
          let errorMsg = switch (e) {
            case (#InsufficientAllowance({ allowance })) {
              "Insufficient spending allowance. Current: " # Nat.toText(allowance / 100_000_000) # " ICP. Cost: " # Nat.toText(requiredIcp / 100_000_000) # " ICP. Please approve more ICP first.";
            };
            case (#InsufficientFunds({ balance })) {
              "Insufficient ICP balance. Current: " # Nat.toText(balance / 100_000_000) # " ICP. Required: " # Nat.toText(requiredIcp / 100_000_000) # " ICP.";
            };
            case (_) {
              "Payment failed. Please check your ICRC-2 approval and ICP balance.";
            };
    ...
    // Deduct parts (prefer universal parts first, then evenly from others)
    let currentInventory = garageManager.getUserInventory(caller);
    var remainingParts = requiredParts;

    // Calculate deductions from each part type
    let universalDeduct = Nat.min(remainingParts, currentInventory.universalParts);
    remainingParts -= universalDeduct;

    // Distribute remaining across other part types
    var speedDeduct : Nat = 0;
    var powerDeduct : Nat = 0;
    var thrusterDeduct : Nat = 0;
    var gyroDeduct : Nat = 0;

    if (remainingParts > 0) {
      let perType = remainingParts / 4;
      let remainder = remainingParts % 4;

      speedDeduct := Nat.min(perType + (if (remainder > 0) { 1 } else { 0 }), currentInventory.speedChips);
      remainingParts -= speedDeduct;

      powerDeduct := Nat.min(perType + (if (remainder > 1) { 1 } else { 0 }), currentInventory.powerCoreFragments);
      remainingParts -= Nat.min(powerDeduct, remainingParts);

      thrusterDeduct := Nat.min(perType + (if (remainder > 2) { 1 } else { 0 }), currentInventory.thrusterKits);
      remainingParts -= Nat.min(thrusterDeduct, remainingParts);

      gyroDeduct := Nat.min(currentInventory.gyroModules, remainingParts);
    };

    // Apply all deductions at once
    let newInventory : PokedBotsGarage.UserInventory = {
      owner = caller;
      speedChips = currentInventory.speedChips - speedDeduct;
      powerCoreFragments = currentInventory.powerCoreFragments - powerDeduct;
      thrusterKits = currentInventory.thrusterKits - thrusterDeduct;
      gyroModules = currentInventory.gyroModules - gyroDeduct;
      universalParts = currentInventory.universalParts - universalDeduct;
    };
    garageManager.setUserInventory(caller, newInventory);

    // Create the new bay
    switch (garageManager.purchaseRepairBaySlot(caller, requiredParts, requiredIcp)) {
      case (#ok(bayId)) {
        #ok({
          message = "🔧 Repair Bay #" # Nat.toText(bayId) # " constructed! You now have " # Nat.toText(bayId) # "/" # Nat.toText(PokedBotsGarage.MAX_REPAIR_BAYS) # " bays.";
          bayId = bayId;
          totalBays = bayId;
        });
      };
  ...
  public shared ({ caller }) func web_upgrade_repair_bay(
    ...
    switch (bay.currentBotToken) {
      case (?_) {
        return #err("Cannot upgrade while a bot is in the bay. Remove the bot first.");
      };
      case (null) {};
    };

    let nextTier = bay.tier + 1;
    let config = switch (PokedBotsGarage.getRepairBayTierConfig(nextTier)) {
      case (?c) { c };
    ...
    let inventory = garageManager.getUserInventory(caller);
    let totalParts = inventory.speedChips + inventory.powerCoreFragments + inventory.thrusterKits + inventory.gyroModules + inventory.universalParts;
    if (totalParts < config.partsCost) {
      return #err("Insufficient parts: need " # Nat.toText(config.partsCost) # ", have " # Nat.toText(totalParts));
    };

    // Process ICP payment if required
    if (config.icpCostE8s > 0) {
      let ledgerId = switch (icpLedgerCanisterId) {
        case (?id) { id };
        case (null) { return #err("ICP Ledger not configured") };
      };
      ...
      switch (transferResult) {
        case (#Err(e)) {
          let errorMsg = switch (e) {
            case (#InsufficientAllowance({ allowance })) {
              "Insufficient spending allowance. Current: " # Nat.toText(allowance / 100_000_000) # " ICP. Cost: " # Nat.toText(config.icpCostE8s / 100_000_000) # " ICP. Please approve more ICP first.";
            };
            case (#InsufficientFunds({ balance })) {
              "Insufficient ICP balance. Current: " # Nat.toText(balance / 100_000_000) # " ICP. Required: " # Nat.toText(config.icpCostE8s / 100_000_000) # " ICP.";
            };
            case (_) {
              "Payment failed. Please check your ICRC-2 approval and ICP balance.";
            };
    ...
    // Deduct parts (prefer universal parts first, then evenly from others)
    let currentInventory = garageManager.getUserInventory(caller);
    var remainingParts = config.partsCost;
    ...
    // Start the upgrade
    let now = Time.now();
    switch (garageManager.startRepairBayUpgrade(caller, bayId, config.partsCost, config.icpCostE8s, now)) {
      case (#ok(completionTime)) {
        #ok({
          message = "🔧 Upgrading Bay #" # Nat.toText(bayId) # " to " # config.name # "! Completes in " # Int.toText(config.buildTimeNanos / 3_600_000_000_000) # " hours.";
          bayId = bayId;
          newTier = nextTier;
          newTierName = config.name;
  ...
  public shared ({ caller }) func web_complete_repair_bay_upgrade(
    ...
    switch (garageManager.completeRepairBayUpgrade(caller, bayId, now)) {
      case (#ok(newTier)) {
        let config = switch (PokedBotsGarage.getRepairBayTierConfig(newTier)) {
          case (?c) { c };
          case (null) { return #err("Invalid tier configuration") };
        };

        #ok({
          message = "✨ Bay #" # Nat.toText(bayId) # " upgraded to " # config.name # "! Now repairs at " # Nat.toText(config.repairRatePerHour) # " cond/hr.";
          bayId = bayId;
          newTier = newTier;
          newTierName = config.name;
          newRepairRate = config.repairRatePerHour;
        });
      };
      case (#err(e)) { #err(e) };
    };
  ...
  public query func web_get_repair_bay_tiers() : async [{
    tier : Nat;
    name : Text;
    repairRatePerHour : Nat;
    powerDrawWatts : Nat;
    ...
    Array.map<PokedBotsGarage.RepairBayTierConfig, { tier : Nat; name : Text; repairRatePerHour : Nat; powerDrawWatts : Nat; partsCost : Nat; icpCostE8s : Nat; buildTimeSeconds : Int }>(
      PokedBotsGarage.REPAIR_BAY_TIERS,
      func(c : PokedBotsGarage.RepairBayTierConfig) : {
        tier : Nat;
        name : Text;
        repairRatePerHour : Nat;
        powerDrawWatts : Nat;
      ...
      } {
        {
          tier = c.tier;
          name = c.name;
          repairRatePerHour = c.repairRatePerHour;
          powerDrawWatts = c.powerDrawWatts;
          partsCost = c.partsCost;
          icpCostE8s = c.icpCostE8s;
          buildTimeSeconds = c.buildTimeNanos / 1_000_000_000;
        };
      },
    );
  };
  ...
  public shared ({ caller }) func web_full_maintenance(
    ...
    switch (transferResult) {
      ...
      case (#Ok(_blockIndex)) {
        ...
        if (perfectTuneUp and resonance.inPeakZone) {
          message := message # "\n✨🔮 PEAK RESONANCE Perfect Tune-Up! ALL penalties removed - pure Speed/Accel boost!";
        } else if (perfectTuneUp and resonance.inGoodZone) {
          message := message # "\n✨ Good Resonance Tune-Up! 70% of penalties removed - mostly pure Speed/Accel boost!";
        } else if (perfectTuneUp and resonance.inWeakZone) {
          message := message # "\n🔧🔧 Weak Resonance Tune-Up (70% fallback)! 30% of penalties removed - minor benefit.";
        } else if (hasOvercharge) {
          message := message # "\n⚠️ Outside resonance window (optimal: " # Nat.toText(resonance.optimalPoint) # "% or 70% fallback) - overcharge penalties remain active.";
        };
  ...
  public shared ({ caller }) func web_combine_parts_to_universal(
    ...
    switch (garageManager.combinePartsToUniversal(caller, amount)) {
      case (#err(e)) { #err(e) };
      case (#ok()) {
        #ok("✅ Combined " # Nat.toText(amount) # " of each part type → " # Nat.toText(amount) # " Universal Parts");
      };
    };
  };
  ...
  // ===========================
  // WEB BATTERY FUNCTIONS
  // ===========================

  /// Get user's battery storage (batteries + heat status for bots)
  public shared query ({ caller }) func web_get_batteries() : async {
    batteries : [{
      id : Nat;
      batteryType : Text;
      healthPercent : Nat;
      storedKwh : Float;
      maxCapacityKwh : Float;
    ...
    let now = Time.now();
    // Use projection for query - don't mutate state
    let storage = garageManager.getBatteryStorage(caller);
    let surplus = garageManager.getGridSurplusForBatteries(caller);

    var totalStored : Float = 0.0;
    var totalCapacity : Float = 0.0;
    var operationalCount : Nat = 0;

    let batteryList = Array.map<PokedBotsGarage.Battery, { id : Nat; batteryType : Text; healthPercent : Nat; storedKwh : Float; maxCapacityKwh : Float; baseCapacityKwh : Float; cyclesPercent : Float; kwhThroughput : Float; discoveredAt : Int; totalJoltsDelivered : Nat; isOperational : Bool; isEnabled : Bool }>(
      storage.batteries,
      func(b) {
        ...
        let operational = PokedBotsGarage.isBatteryOperational(b);

        // Project current charge without mutating state
        let projectedCharge = garageManager.projectBatteryCharge(b, surplus, now);

        totalStored += projectedCharge;
        totalCapacity += maxCapacity; // Use effective capacity (cycle-degraded), not base
        if (operational) { operationalCount += 1 };

        {
          id = b.id;
          batteryType = typeText;
          healthPercent = b.healthPercent;
          storedKwh = projectedCharge;
          maxCapacityKwh = maxCapacity;
          baseCapacityKwh = baseCapacity;
          cyclesPercent = cycles;
...

    {
      ...
      summary = {
        totalBatteries = batteryList.size();
  ...
  public shared query func web_get_bot_heat(tokenIndex : Nat) : async {
    heatStacks : Nat;
    maxHeat : Nat;
    lastJoltTime : Int;
    overheatUntil : ?Int;
    ...
    let now = Time.now();
    let heat = garageManager.getCurrentBotHeat(tokenIndex, now);
    let status = garageManager.getBotHeatStatus(tokenIndex);

    let overheated = switch (status.overheatUntil) {
      case (?until) { now < until };
      case (null) { false };
    };

    let minutesLeft : ?Nat = switch (status.overheatUntil) {
      case (?until) {
        if (now < until) {
          // Safe to use abs since we know until > now in this branch
          let diffNanos : Nat = Int.abs(until - now);
          let divisor : Nat = 60_000_000_000;
          let minutes : Nat = diffNanos / divisor;
          ?minutes;
        } else { null };
      };
      case (null) { null };
...

    {
      heatStacks = heat;
      maxHeat = 4;
      lastJoltTime = status.lastJoltTime;
      overheatUntil = status.overheatUntil;
      isOverheated = overheated;
      minutesUntilCooldown = minutesLeft;
    };
  };

  /// Jolt a bot using a battery
  public shared ({ caller }) func web_jolt_bot(
    batteryId : Nat,
    tokenIndex : Nat,
    ...
    switch (garageManager.joltBot(caller, batteryId, tokenIndex, now)) {
      case (#ok(result)) {
        #ok({
          ...
          newBotBattery = result.newBotBattery;
          newBatteryCharge = result.newBatteryCharge;
          newBatteryHealth = result.newBatteryHealth;
          newHeatStacks = result.newHeatStacks;
  ...
  public shared ({ caller }) func web_repair_battery(
    ...
    switch (garageManager.repairBattery(caller, batteryId)) {
      case (#ok(result)) {
        #ok({
          healthGained = result.healthGained;
          newHealth = result.newHealth;
          cyclesPercent = result.cycles;
  ...
  public shared ({ caller }) func web_rebuild_battery(
    batteryId : Nat,
    useIcp : Bool,
  ) : async Result.Result<Text, Text> {
    // Track this method call
    ...
    if (useIcp) {
      ...
      let battery = switch (garageManager.getBattery(caller, batteryId)) {
        case (?b) { b };
        case (null) { return #err("Battery not found") };
      };
      ...
      let transferResult = try {
        ...
        return #err("ICP transfer failed: " # Error.message(e));
      };
      ...
      switch (transferResult) {
        case (#Err(err)) {
          let errMsg = switch (err) {
            case (#InsufficientAllowance { allowance }) {
              "Insufficient allowance: " # Nat.toText(allowance) # " e8s. Need " # Nat.toText(icpCost + TRANSFER_FEE) # " e8s approved.";
            };
            case (#InsufficientFunds { balance }) {
        ...
        case (#Ok(_)) {
          // Payment successful, do rebuild
          switch (garageManager.rebuildBatteryCore(caller, batteryId, true)) {
            case (#ok(result)) { #ok(result.message) };
            case (#err(e)) { #err("Rebuild failed after payment: " # e) };
          };
        };
      };
    } else {
      // Parts payment
      switch (garageManager.rebuildBatteryCore(caller, batteryId, false)) {
        case (#ok(result)) { #ok(result.message) };
        case (#err(e)) { #err(e) };
      };
    };
  ...
  public shared ({ caller }) func web_salvage_battery(
    ...
    switch (garageManager.salvageBattery(caller, batteryId)) {
      case (#ok(result)) {
        ...
        #ok({
          partsReturned = result.partsReturned;
          batteryType = typeText;
        });
      };
      case (#err(e)) { #err(e) };
  ...
  public shared ({ caller }) func web_toggle_battery(
    ...
    switch (garageManager.toggleBatteryEnabled(caller, batteryId, now)) {
      case (#ok(newState)) {
        let stateText = if (newState) { "enabled" } else { "disabled" };
        #ok({
          isEnabled = newState;
          message = "Battery charging " # stateText;
        });
      };
      case (#err(e)) { #err(e) };
    };
  ...
  public query func web_get_battery_info() : async {
    types : [{
      name : Text;
      baseCapacityKwh : Float;
      repairCostParts : Nat;
      rebuildCostParts : Nat;
      rebuildCostIcp : Nat;
      salvageReturnParts : Nat;
      drawRateWatts : Nat;
    }];
  } {
    {
      types = [
        {
          name = "ScrapCell";
          baseCapacityKwh = PokedBotsGarage.getBaseBatteryCapacity(#ScrapCell);
          repairCostParts = PokedBotsGarage.getBatteryRepairCost(#ScrapCell);
          rebuildCostParts = PokedBotsGarage.getBatteryRebuildCost(#ScrapCell);
          rebuildCostIcp = PokedBotsGarage.getBatteryRebuildIcpCost(#ScrapCell);
          salvageReturnParts = PokedBotsGarage.getBatterySalvageReturn(#ScrapCell);
          drawRateWatts = PokedBotsGarage.getBatteryDrawRate(#ScrapCell);
        },
        {
          name = "SalvagePack";
          baseCapacityKwh = PokedBotsGarage.getBaseBatteryCapacity(#SalvagePack);
          repairCostParts = PokedBotsGarage.getBatteryRepairCost(#SalvagePack);
          rebuildCostParts = PokedBotsGarage.getBatteryRebuildCost(#SalvagePack);
          rebuildCostIcp = PokedBotsGarage.getBatteryRebuildIcpCost(#SalvagePack);
          salvageReturnParts = PokedBotsGarage.getBatterySalvageReturn(#SalvagePack);
          drawRateWatts = PokedBotsGarage.getBatteryDrawRate(#SalvagePack);
        },
        {
          name = "IndustrialBank";
          baseCapacityKwh = PokedBotsGarage.getBaseBatteryCapacity(#IndustrialBank);
          repairCostParts = PokedBotsGarage.getBatteryRepairCost(#IndustrialBank);
          rebuildCostParts = PokedBotsGarage.getBatteryRebuildCost(#IndustrialBank);
          rebuildCostIcp = PokedBotsGarage.getBatteryRebuildIcpCost(#IndustrialBank);
          salvageReturnParts = PokedBotsGarage.getBatterySalvageReturn(#IndustrialBank);
          drawRateWatts = PokedBotsGarage.getBatteryDrawRate(#IndustrialBank);
        },
        {
          name = "PlasmaVault";
          baseCapacityKwh = PokedBotsGarage.getBaseBatteryCapacity(#PlasmaVault);
          repairCostParts = PokedBotsGarage.getBatteryRepairCost(#PlasmaVault);
          rebuildCostParts = PokedBotsGarage.getBatteryRebuildCost(#PlasmaVault);
          rebuildCostIcp = PokedBotsGarage.getBatteryRebuildIcpCost(#PlasmaVault);
          salvageReturnParts = PokedBotsGarage.getBatterySalvageReturn(#PlasmaVault);
          drawRateWatts = PokedBotsGarage.getBatteryDrawRate(#PlasmaVault);
        },
      ];
    };
  };
  ...
  public query func web_get_bot_loadout(
    tokenIndex : Nat
  ) : async {
    tokenIndex : Nat;
    legs : ?GearSystem.GearPiece;
    thruster : ?GearSystem.GearPiece;
    chassis : ?GearSystem.GearPiece;
    gyro : ?GearSystem.GearPiece;
    core : ?GearSystem.GearPiece;
    module_ : ?GearSystem.GearPiece;
    consumable1 : ?GearSystem.ConsumableInstance;
    consumable2 : ?GearSystem.ConsumableInstance;
  } {
    gearManager.getResolvedLoadout(tokenIndex);
  };

  /// Get total gear stat bonuses for a bot
  public query func web_get_gear_bonuses(
    tokenIndex : Nat
  ) : async {
    speed : Nat;
    powerCore : Nat;
  ...
  public query func web_get_player_gear(
    owner : Principal
  ) : async [GearSystem.GearPiece] {
    gearManager.getPlayerGear(owner);
  };

  /// Get which bot each gear piece is equipped on (gearId → tokenIndex)
  public query func web_get_gear_equip_map() : async [(Nat, Nat)] {
    gearManager.getGearEquipMap();
  };

  /// Get a single gear piece by ID
  public query func web_get_gear_piece(
    gearId : Nat
  ) : async ?GearSystem.GearPiece {
    gearManager.getGearPiece(gearId);
  };

  /// Get total gear count (admin/stats)
  public query func web_get_total_gear_count() : async Nat {
    gearManager.getTotalGearCount();
  };

  /// Equip a gear piece to a bot
  public shared ({ caller }) func web_equip_gear(
    tokenIndex : Nat,
    gearId : Nat,
  ) : async Result.Result<GearSystem.BotLoadout, Text> {
    trackMethodCall("web_equip_gear", caller);
  ...
  public shared ({ caller }) func web_unequip_gear(
    tokenIndex : Nat,
    slot : GearSystem.GearSlot,
  ) : async Result.Result<GearSystem.BotLoadout, Text> {
    trackMethodCall("web_unequip_gear", caller);
  ...
  public shared ({ caller }) func web_craft_gear(
    tokenIndex : Nat,
    gearIds : [Nat],
  ) : async Result.Result<GearSystem.GearPiece, Text> {
    trackMethodCall("web_craft_gear", caller);
    ...
    let currentSeason = Leaderboard.getSeasonIdFromTime(Time.now());
    let currentIlvl = GearSystem.ilvlFromSeason(currentSeason);
    // Deterministic seed from gear IDs
    var seed : Nat = 0;
    for (id in gearIds.vals()) {
      seed := seed * 31 + id;
    };

    gearManager.craftGear(caller, tokenIndex, gearIds, currentSeason, currentIlvl, seed);
  };

  // ===== CONSUMABLE ENDPOINTS =====

  /// Get all consumables owned by a player
  public query func web_get_player_consumables(owner : Principal) : async [GearSystem.ConsumableInstance] {
    let inv = gearManager.getPlayerInventory(owner);
    let buf = Buffer.Buffer<GearSystem.ConsumableInstance>(inv.consumables.size());
    for (id in inv.consumables.vals()) {
      switch (gearManager.getConsumableInstance(id)) {
        case (?inst) { buf.add(inst) };
        case (null) {};
      };
    };
    Buffer.toArray(buf);
  ...
  public shared ({ caller }) func web_equip_consumable(
    tokenIndex : Nat,
    instanceId : Nat,
    slot : Nat,
  ) : async Result.Result<GearSystem.BotLoadout, Text> {
    trackMethodCall("web_equip_consumable", caller);

    // Verify bot ownership
    switch (garageManager.getStats(tokenIndex)) {
      case (?stats) {
        if (not Principal.equal(stats.ownerPrincipal, caller)) {
          return #err("Not your bot");
        };
      };
      case (null) { return #err("Bot not found") };
    };

    gearManager.equipConsumable(caller, tokenIndex, instanceId, slot);
  };

  /// Unequip a consumable from a bot's loadout (slot 1 or 2)
  public shared ({ caller }) func web_unequip_consumable(
    tokenIndex : Nat,
    slot : Nat,
  ) : async Result.Result<GearSystem.BotLoadout, Text> {
    trackMethodCall("web_unequip_consumable", caller);
    ...
    var loadout = gearManager.getLoadout(tokenIndex);
    if (slot == 1) {
      loadout := { loadout with consumable1 = null; lastModified = Time.now() };
    } else if (slot == 2) {
      loadout := { loadout with consumable2 = null; lastModified = Time.now() };
    } else {
      return #err("Consumable slot must be 1 or 2");
    };

    gearManager.setLoadout(tokenIndex, loadout);
    #ok(loadout);
  };
};
