    ...
    func(_args : McpTypes.JsonValue, _auth : ?AuthTypes.AuthInfo, cb : (Result.Result<McpTypes.CallToolResult, McpTypes.HandlerError>) -> ()) : async () {
      ...
      switch (garage.getStats(tokenIndex)) {
        ...
        case (?botStats) {
          ...
          // Start continuous mission (no duration for MCP tool - always continuous)
          let now = Time.now();
          switch (garage.startScavengingMission(tokenIndex, zone, now, null)) {
            case (#err(e)) {
            ...
            case (#ok(_)) {
              ...
              let response = Json.obj([
                ...
                ("message", Json.str("🔧 Bot sent out to scavenge in " # zoneDesc # ". Rewards accumulate every 15 minutes with randomized part distribution. Retrieve bot anytime with garage_complete_scavenging. WARNING: Bot dies at 0 battery OR condition = lose ALL pending rewards!")),
              ]);
