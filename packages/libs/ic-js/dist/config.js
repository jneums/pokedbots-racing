...
export const getCanisterId = (name) => {
    if (!_isConfigured) {
        throw new Error('The @pokedbots-racing/ic-js package has not been configured. Please call the configure() function at application startup.');
    }
    const canisterId = _canisterIds[name.toUpperCase()];
    if (!canisterId) {
        console.error('Available canister names in config:', Object.keys(_canisterIds));
        console.error(`Requested canister name: '${name}'`);
        throw new Error(`Configuration does not contain a canister ID for '${name}'.`);
    }
    return canisterId;
};
/**
