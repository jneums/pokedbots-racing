...
def analyze_acceleration_impact(bot: Bot):
    ...
    print(f"   Accel 45: {terrain_penalty_45:.4f}x time ({(terrain_penalty_45/terrain_penalty_51 - 1)*100:+.2f}%)")
    print(f"   Accel 40: {terrain_penalty_40:.4f}x time ({(terrain_penalty_40/terrain_penalty_51 - 1)*100:+.2f}%)")
    print(f"   Accel 35: {terrain_penalty_35:.4f}x time ({(terrain_penalty_35/terrain_penalty_51 - 1)*100:+.2f}%)")
    ...
    print(f"   Balanced (A:45 S:45): {short_bonus_45_45:.4f}x time ({(short_bonus_45_45/short_bonus_51_41 - 1)*100:+.2f}%)")
    print(f"   Speed specialist (A:40 S:50): {short_bonus_40_50:.4f}x time ({(short_bonus_40_50/short_bonus_51_41 - 1)*100:+.2f}%)")
...
def detailed_rivalry_analysis(bot1: Bot, bot2: Bot):
    ...
    print("-" * 80)
    print(f"{'Speed':20} {bot1.stats.speed:<15} {bot2.stats.speed:<15} {bot1.stats.speed - bot2.stats.speed:+d} (favors {bot1.id if bot1.stats.speed > bot2.stats.speed else bot2.id})")
    print(f"{'Power Core':20} {bot1.stats.powerCore:<15} {bot2.stats.powerCore:<15} {bot1.stats.powerCore - bot2.stats.powerCore:+d} (favors {bot1.id if bot1.stats.powerCore > bot2.stats.powerCore else bot2.id})")
    print(f"{'Acceleration':20} {bot1.stats.acceleration:<15} {bot2.stats.acceleration:<15} {bot1.stats.acceleration - bot2.stats.acceleration:+d} (TIED)")
    print(f"{'Stability':20} {bot1.stats.stability:<15} {bot2.stats.stability:<15} {bot1.stats.stability - bot2.stats.stability:+d} (TIED)")
    print(f"{'TOTAL':20} {bot1.total_stats:<15} {bot2.total_stats:<15} {bot1.total_stats - bot2.total_stats:+d} (favors {bot1.id if bot1.total_stats > bot2.total_stats else bot2.id})")
    ...
    total_bot1 = sum(1 for t in terrains for d in distances if wins_by_scenario[f"{t}_{d}km"]["bot1"] > wins_by_scenario[f"{t}_{d}km"]["bot2"])
    total_bot2 = sum(1 for t in terrains for d in distances if wins_by_scenario[f"{t}_{d}km"]["bot2"] > wins_by_scenario[f"{t}_{d}km"]["bot1"])
    ...
    print(f"  {bot1.id}: {bot1_total_wins} wins ({bot1_total_wins/total_races*100:.1f}%)")
    print(f"  {bot2.id}: {bot2_total_wins} wins ({bot2_total_wins/total_races*100:.1f}%)")
    ...
    if bot1_total_wins > bot2_total_wins:
        print(f"  {bot1.id} wins overall with a {bot1_total_wins - bot2_total_wins} race advantage")
        print(f"  Key advantages: +{bot1.stats.powerCore - bot2.stats.powerCore} power core helps on WastelandSand")
    else:
        print(f"  {bot2.id} wins overall with a {bot2_total_wins - bot1_total_wins} race advantage")
        print(f"  Key advantages: +{bot2.stats.speed - bot1.stats.speed} speed helps everywhere, especially short races")
...
def main():
    ...
    print(f"  {bot_4829.id}: Total {bot_4829.total_stats} [S:{bot_4829.stats.speed} P:{bot_4829.stats.powerCore} A:{bot_4829.stats.acceleration} St:{bot_4829.stats.stability}]")
    print(f"  {bot_6096.id}: Total {bot_6096.total_stats} [S:{bot_6096.stats.speed} P:{bot_6096.stats.powerCore} A:{bot_6096.stats.acceleration} St:{bot_6096.stats.stability}]")
    ...
    for group_name, bots in accel_groups.items():
        ...
        print(f"  Best terrain: {max(wins_by_terrain, key=wins_by_terrain.get)} "
              f"({wins_by_terrain[max(wins_by_terrain, key=wins_by_terrain.get)]}/{total//3})")
        print(f"  Best distance: {max(wins_by_distance, key=wins_by_distance.get)}km "
