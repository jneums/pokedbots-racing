...
def main():
    ...
    print(f"Stats: Speed {target_bot.stats.speed}, Power {target_bot.stats.powerCore}, "
    ...
    tier_bots = [b for b in all_bots if tier_min <= b.total_stats <= tier_max and b.token_id != target_bot.token_id]
    ...
    balanced_tier = sorted(
        [b for b in tier_bots 
         if min(b.stats.speed, b.stats.powerCore, b.stats.acceleration, b.stats.stability) >= 40],
    ...
    for i, bot in enumerate(high_accel_tier[:10], 1):
        print(f"  {i:2}. {bot.id:8} A:{bot.stats.acceleration:2} Total:{bot.total_stats:3} "
    ...
    print(f"{'Rank':<6}{'Bot':<10}{'Wins':<8}{'Races':<8}{'Win%':<8}{'Total':<8}{'Accel':<8}{'Faction'}")
    print("-" * 90)
    
    target_rank = None
    for rank, (bot, wins, races, win_rate) in enumerate(rankings[:30], 1):
        marker = " ⭐" if bot.token_id == target_bot.token_id else ""
        print(f"{rank:<6}{bot.id:<10}{wins:<8}{races:<8}{win_rate:5.1f}%  {bot.total_stats:<8}"
    ...
    print(f"{'Opponent':<10}{'Win%':<10}{'Record':<12}{'Opp Total':<12}{'Opp Accel':<12}{'Faction'}")
    print("-" * 90)
    for opponent, win_rate, wins, total in matchup_results[:10]:
        print(f"{opponent.id:<10}{win_rate:5.1f}%    {wins:3}-{total-wins:3}      "
...
def run_simulation(target_bot: Bot, opponents: List[Bot], num_races: int = 500):
    ...
    print(f"  Podiums: {positions.get(1,0)+positions.get(2,0)+positions.get(3,0)} "
    ...
    for rank, (token_id, wins) in enumerate(top_competitors, 1):
        bot = next((b for b in [target_bot] + opponents if b.token_id == token_id), None)
        if bot:
            rate = (wins / num_races) * 100
            marker = " ⭐" if token_id == target_bot.token_id else ""
            print(f"    {rank}. {bot.id:8} {wins:3} wins ({rate:5.1f}%) | "
