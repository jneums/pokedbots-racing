...
def analyze_bot_strengths(bot: Bot):
    ...
    print(
        f"  Power Core:   {stats.powerCore:2} ({stats.powerCore/total*100:5.1f}% of total)"
    )
    print(
        f"  Acceleration: {stats.acceleration:2} ({stats.acceleration/total*100:5.1f}% of total) ⭐ ULTRA RARE"
    )
    print(
        f"  Stability:    {stats.stability:2} ({stats.stability/total*100:5.1f}% of total)"
    )

    print(f"\n💪 STRENGTHS:")
    if stats.acceleration > 48:
        print(
            f"  • Elite acceleration ({stats.acceleration}) - excellent for short races & MetalRoads"
        )
    if stats.powerCore > 44:
        print(
            f"  • Strong power core ({stats.powerCore}) - good for WastelandSand terrain"
        )
    if stats.speed + stats.acceleration > 90:
        print(
            f"  • Speed+Accel combo: {stats.speed + stats.acceleration} - STAT SYNERGY BONUS (5% time reduction)"
        )
    ...
    if stats.stability < 45:
        print(
            f"  • Moderate stability ({stats.stability}) - not ideal for ScrapHeaps terrain"
        )
...
def find_comparable_bots(all_bots: List[Bot], target_bot: Bot) -> Dict[str, List[Bot]]:
    ...
    blackhole_bots = [
        b
        for b in all_bots
        if b.faction == "Blackhole" and b.token_id != target_bot.token_id
    ]
    top_blackhole = sorted(blackhole_bots, key=lambda b: b.total_stats, reverse=True)[
        :10
    ]
    ...
    balanced_elite = [
        ...
        for b in all_bots
        if b.stats.speed >= 40
        and b.stats.powerCore >= 40
        and b.stats.acceleration >= 40
        and b.stats.stability >= 40
        and b.token_id != target_bot.token_id
    ][:15]
...
def run_matchup_analysis(
    bot: Bot, opponents: List[Bot], category_name: str, num_races: int = 500
):
    ...
    for rank, (token_id, wins) in enumerate(top_performers, 1):
        ...
        print(
            f"  {rank}. {racer.id:20} {wins:3} wins ({rate:5.1f}%) | Total Stats: {racer.total_stats}{marker}"
        )
    ...
    for terrain in terrains:
        ...
        print(
            f"  {terrain:20} {terrain_wins:3}/{terrain_races:3} ({rate:5.1f}%) {analysis}"
        )
    ...
    for distance in distances:
        ...
        if distance < 10:
            ...
            analysis = (
                f"(Long race: Power+Stab = {bot.stats.powerCore + bot.stats.stability})"
            )

        print(
            f"  {distance:2}km: {distance_wins:3}/{distance_races:3} ({rate:5.1f}%) {analysis}"
        )
    ...
    return {
        ...
        "best_distance": max(
            distances, key=lambda d: wins_by_distance[bot.token_id][d]
        ),
    }
...
def detailed_1v1_analysis(bot: Bot, opponent: Bot, num_races: int = 200):
    ...
    print("-" * 80)
    print(
        f"{'Speed':<15} {bot.stats.speed:<15} {opponent.stats.speed:<15} {'+' + str(bot.stats.speed - opponent.stats.speed) if bot.stats.speed > opponent.stats.speed else str(bot.stats.speed - opponent.stats.speed)}"
    )
    print(
        f"{'Power Core':<15} {bot.stats.powerCore:<15} {opponent.stats.powerCore:<15} {'+' + str(bot.stats.powerCore - opponent.stats.powerCore) if bot.stats.powerCore > opponent.stats.powerCore else str(bot.stats.powerCore - opponent.stats.powerCore)}"
    )
    print(
        f"{'Acceleration':<15} {bot.stats.acceleration:<15} {opponent.stats.acceleration:<15} {'+' + str(bot.stats.acceleration - opponent.stats.acceleration) if bot.stats.acceleration > opponent.stats.acceleration else str(bot.stats.acceleration - opponent.stats.acceleration)}"
    )
    print(
        f"{'Stability':<15} {bot.stats.stability:<15} {opponent.stats.stability:<15} {'+' + str(bot.stats.stability - opponent.stats.stability) if bot.stats.stability > opponent.stats.stability else str(bot.stats.stability - opponent.stats.stability)}"
    )
    print(
        f"{'TOTAL':<15} {bot.total_stats:<15} {opponent.total_stats:<15} {'+' + str(bot.total_stats - opponent.total_stats) if bot.total_stats > opponent.total_stats else str(bot.total_stats - opponent.total_stats)}"
    )
...
def main():
    ...
    for category, bots in comparable.items():
        ...
        for bot in bots[:5]:
            print(
                f"  {bot.id:20} Total:{bot.total_stats:3} | S:{bot.stats.speed:2} P:{bot.stats.powerCore:2} A:{bot.stats.acceleration:2} St:{bot.stats.stability:2} | {bot.faction}"
            )
    ...
    matchup_results["top_overall"] = run_matchup_analysis(
        target_bot, comparable["top_overall"][:15], "Top 15 Bots Overall", num_races=600
    )
    ...
    if len(comparable["high_acceleration"]) > 1:
        high_accel_rival = next(
            (
                b
                for b in comparable["high_acceleration"]
                if b.token_id != target_bot.token_id
            ),
            None,
        )
        if high_accel_rival:
    ...
    for category, results in matchup_results.items():
        print(
            f"  {category.replace('_', ' ').title():30} {results['win_rate']:5.1f}% win rate"
        )
    ...
    print(f"\n  🎯 COMPETITIVE TIER:")
    avg_win_rate = sum(r["win_rate"] for r in matchup_results.values()) / len(
        matchup_results
    )
    if avg_win_rate > 50:
        tier = "ELITE (Top 1%)"
