...
module {
  public func config() : McpTypes.Tool = {
    ...
    description = ?"Browse available PokedBots NFTs for sale. Returns listings sorted by price (lowest first). Use 'after' with a token index to continue from that point (20 listings per call).";
    payment = null;
    inputSchema = Json.obj([
      ...
      ("properties", Json.obj([
        ("after", Json.obj([
          ...
          ("description", Json.str("Show listings after this token index (optional)"))
        ]))
      ])),
    ]);
    outputSchema = null;
  ...
  public func handle(context : ToolContext.ToolContext) : (
    ...
    func(_args : McpTypes.JsonValue, _auth : ?AuthTypes.AuthInfo, cb : (Result.Result<McpTypes.CallToolResult, McpTypes.HandlerError>) -> ()) : async () {
      ...
      // Get page slice
      let pageListings = Array.tabulate<Text>(endIdx - startIdx, func(i) {
        let idx = startIdx + i;
